...
public class FileMaina09 {

	public static void main(String[] args) {
		// 파일, 폴더(디렉토리) 생성, 삭제, 관리
		
		
		// System.get.Property(key) : Java Runtime이 실행 중인 컴퓨터의 여러가지 시스템 정보들을 리턴.
		 ...
		 *  (예)C:\workspace\lab-java\java13_File\data\encoding-anso.txt
		 * 
		 * 상대경로 (relative path)
		 *  현재 작업 디렉토리(CWD)를 기준으로 파일 또는 상대적으로 찾아가는 방식.
		 ...
		 */
		// backslash가 포함된 문자열		
		System.out.println("C:\\workspace\\lab-java\\java13_File\\data\\encoding-anso.txt");	
		...
		// -> 파일 객체를 생성할 때 절대 경로 생성하면 , getPath() 메서드는 절대 경로를 리턴.
		...
		System.out.println("크기? " + f.length()); // 바이트 (byte)
		...
		// 기존 폴더 삭제 (1) File 객체 생성, (2) 폴더가 없으면, delete() 메서드 호출.
		if(newFolder.exists()) {
			newFolder.delete();
