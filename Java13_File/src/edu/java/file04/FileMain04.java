...
/*
 * FIS, FOS 와 같은 클래스들의 객체들은 사용한 이후에 반드시 close() 메서드를 호출해서 리소스를 해체해야만 함.
 * Try-Catch-finally 구문에서는.
 * 리소스 생성은 try 블록에서 하고, 리소스 해체는 finally 블록에서 작성함.
 * 실수로 close 메서드를 호출하지 않는 경우들이 생길 수 있음.
 * 리소스만 생성하면 close 메서들을 자동으로 호출해 주는 문법이 필요.\
...
public class FileMain04 {
	...
	public static void main(String[] args) {
		...
		try ( // close 메서드를 호출해야 하는 변수들만 선언, 초기화 - > close는 try-catch가 끝난 후 자동으로 호출.
				FileInputStream in = new FileInputStream(origin);
				FileOutputStream out = new FileOutputStream(dest);
				) { 
			while (true) {
				byte[] buf = new byte[10]; // 파일에서 읽을 내용을 저장할 byte 배열
