...
// - 프로그램 로딩 시점(main 메서드 시작 전)에 JRE이 사용하는 메모리 공간 중에서 메서드(Method)
// - 클래스 이름을 사용해서 사용하는 멤버.
// - static 메서드는 static 멤버들은 static 멤버에서만 사용 가능.
// -> 인스턴스 메서드는 인스턴스 멤버 (필드, 메서드)를 사용할 수 없음!
public class ModifierMain07 {
	...
	public static void main(String[] args) {
		// static 메서드는 객체 생성여부와 상관 없이 클래스
		Test.printStaticInfo();
		// static 필드는 객체생성 여부와 상관 없이 클래스 이름을 사용해서 접근(사용)
		Test.y = 123;
		// Test 타입의 객체 생성.
		Test test = new Test();
